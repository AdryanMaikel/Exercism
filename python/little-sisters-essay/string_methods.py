...
def capitalize_title(title: str) -> str:
    ...
    :return: str - title string in title case (first letters
    capitalized).
    """
    return title.title()
...
def check_sentence_ending(sentence: str) -> bool:
    """Check the ending of the sentence to verify that a period is
    present.
...
def clean_up_spacing(sentence: str) -> str:
    """Verify that there isn't any whitespace at the start and end of
    the sentence.
...
def replace_word_choice(sentence: str, old_word: str, new_word: str) -> str:
    ...
    return " ".join([word.replace(old_word, new_word)
                     if old_word in word else word for word in words])
