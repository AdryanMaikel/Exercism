...
def add_me_to_the_queue(express_queue: list[str], normal_queue: list[str],
    ...
    """Add a person to the 'express' or 'normal' queue depending on the
    ticket number.
...
def find_my_friend(queue: list[str], friend_name: str) -> int:
    """Search the queue for a name and return their queue position
    (index).
...
def sorted_names(queue: list[str]) -> list[str]:
    """Sort the names in the queue in alphabetical order and return the
    result.
