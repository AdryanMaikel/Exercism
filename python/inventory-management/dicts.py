...
def create_inventory(items: list[str]) -> dict[str, int]:
    """Create a dict that tracks the amount (count) of each element on
    the `items` list.
...
def decrement_items(inventory: dict[str, int],
...
def remove_item(inventory: dict[str, int], item: str) -> dict[str, int]:
    ...
    :return: dict - updated inventory with item removed. Current
    inventory if item does not match.
    """
    if item in inventory:
        del inventory[item]
    return inventory
...
def list_inventory(inventory: dict[str, int]) -> list[str] | tuple[str]:
    """Create a list containing all (item_name, item_count) pairs in
    inventory.
    ...
    for item, quantity in inventory.items():
        if quantity > 0:
            all_content.append((item, quantity))
    return all_content
